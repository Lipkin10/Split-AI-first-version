console.log('🧪 Testing Supabase CRUD Operations directly...')
/**
...
async function test1_Categories() {
  ...
  try {
    ...
    if (!categories || categories.length < 8) {
      throw new Error(`Expected at least 8 categories, got ${categories?.length}`)
    }
...
async function test2_CreateGroup() {
  ...
  try {
    ...
    console.log(`📊 Participants: ${testParticipants.map(p => p.name).join(', ')}`)
...
async function test8_GetSingleExpense() {
  ...
  try {
    ...
    console.log('✅ Test 8 PASSED - Single expense with relationships retrieved')
...
async function runAllTests() {
  ...
  if (testGroupId) {
    ...
    console.log('You can manually inspect or delete this group from Supabase if needed.')
  }
