...
async function test4_FilteredSubscriptions() {
  ...
  try {
    ...
    if (group1Events >= 1 && group2Events === 0) {
      ...
      console.log(
        `📊 Group 1 events: ${group1Events}, Group 2 events: ${group2Events}`,
      )
      testResults.passed++
    } else {
      throw new Error(
        `Filter not working: Group1=${group1Events}, Group2=${group2Events}`,
      )
    }
    ...
    await supabase
      .from('Expense')
      .delete()
      .in('groupId', [testGroupId1, testGroupId2])
    await supabase
      .from('Participant')
      .delete()
      .in('groupId', [testGroupId1, testGroupId2])
...
async function test5_PerformanceTest() {
  ...
  try {
    ...
    const perfChannel = supabase
      ...
      .on(
        ...
        (payload) => {
          eventsReceived++
          console.log(
            `📡 Performance event ${eventsReceived}/${expectedEvents}`,
          )
        },
      )
      .subscribe()
    ...
    if (successRate >= 80) {
      console.log(
        `✅ Test 5 PASSED - Performance test: ${successRate}% success rate`,
      )
      testResults.passed++
    } else {
