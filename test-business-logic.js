    ...
    if (expense.splitMode === 'EVENLY') {
      ...
      expense.paidFor.forEach((pf) => {
        const shareAmount = Math.round(
          (expense.amount * pf.shares) / totalShares,
        )
        balances[pf.participantId].owes += shareAmount
      })
    }
  ...
  try {
    ...
    participants.forEach((p) => {
      ...
      if (Math.abs(calculated.balance - expected.balance) > 1) {
        ...
        console.error(
          `❌ Balance mismatch for ${p.name}: expected ${expected.balance}, got ${calculated.balance}`,
        )
      } else {
...
async function test4_ComplexScenario() {
  ...
  try {
    ...
    const calculatedBalances = calculateExpectedBalances(
      allExpenses,
      participants,
    )
    ...
    participants.forEach((p) => {
      ...
      if (Math.abs(calculated - expected) > 1) {
        calculationCorrect = false
        console.error(
          `❌ Final balance mismatch for ${p.name}: expected ${expected}, got ${calculated}`,
        )
      } else {
    ...
    const totalBalance = participants.reduce(
      (sum, p) => sum + calculatedBalances[p.id].balance,
      0,
    )
...
async function test5_EdgeCases() {
  ...
  try {
    ...
    await supabase
      .from('ExpensePaidFor')
      .delete()
      .in('expenseId', [smallExpId, largeExpId])
...
async function runBusinessLogicTests() {
  ...
  console.log(
    `📊 Success Rate: ${((testResults.passed / tests.length) * 100).toFixed(
      1,
    )}%`,
  )
